...
<style>
  .command-trigger{font:inherit;background:none;color:var(--text);border:0;border-radius:24px;background:var(--surface);padding:.65rem .85rem;cursor:pointer;display:flex;gap:1.5rem;align-items:center}.command-trigger:hover{border-color:var(--accent);color:var(--accent)}.close:hover{background:var(--yellow)}.command-trigger[hidden]{display:none}kbd{font:11px monospace;color:var(--muted)}
  dialog{width:min(600px,calc(100% - 32px));max-height:85dvh;overscroll-behavior:contain;background:var(--bg);color:var(--text);border:0;border-radius:16px;padding:24px;margin:auto;box-shadow:0 24px 80px #20231f33}dialog::backdrop{background:#20231f88;}
