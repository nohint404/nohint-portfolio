<script lang="ts">
  ...
  let history = $state<{ command: string; reply: string }[]>([]);
  let status = $state('');
  onMount(() => { ready = true; });
  function run(value = command) {
    const reply = terminalReply(value);
    if (value.trim().toLowerCase() === 'clear') history = [];
    else if (value.trim()) history = [...history.slice(-7), { command: value.slice(0, 160), reply }];
...
<div class="terminal">
  ...
  <div class="terminal-history" tabindex="0" role="region" aria-label="Command history">
    ...
    {#each history as entry}<p class="typed-command">$ {entry.command}</p><pre>{entry.reply}</pre>{/each}
  </div>
