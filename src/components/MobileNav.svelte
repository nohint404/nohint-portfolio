...
<button class="mobile-trigger" bind:this={trigger} hidden={!ready} onclick={open} aria-label="Open navigation" aria-haspopup="dialog">Menu</button>
<dialog class="mobile-dialog" bind:this={dialog} aria-labelledby="mobile-title" onclose={() => (previous ?? trigger)?.focus()}>
  <div class="mobile-top"><h2 id="mobile-title">Pick a path.</h2><button onclick={() => dialog.close()} aria-label="Close navigation">Close ×</button></div>
